...
class DeepRecurrentQNetwork(TFBaseModel):
    def __init__(self, env, name, **kwargs):
        ...
        # learning rate default fixed to 1e-4 (was 1e4)
        ...
        self.terminal_buf = np.empty(1, dtype=np.bool_)
    ...
    def _create_network(self, input_state, reuse=None):
        ...
        """
        kernel_num = [32, 32]
        # Support either dict or list/tuple of layer sizes
        if isinstance(self.nn_layers, dict):
            hidden_size = list(self.nn_layers.values())
        elif isinstance(self.nn_layers, (list, tuple)):
            hidden_size = list(self.nn_layers)
        else:
            hidden_size = [int(self.nn_layers)]

        if len(hidden_size) == 1:
            print("1 Layer DRQN NN with " + str(hidden_size[0]) + " neurons")
            dense = tf.layers.dense(input_state, units=hidden_size[0], activation=tf.nn.relu, name="dense")
            ...
            h_state = tf.layers.dense(input_state, units=hidden_size[0], activation=tf.nn.relu, name="h_state")
            dense = tf.layers.dense(h_state, units=hidden_size[1], activation=tf.nn.relu, name="dense")
            state_size = hidden_size[1]
        else:
            # fallback: one small layer
            state_size = hidden_size[-1]
            dense = tf.layers.dense(input_state, units=state_size, activation=tf.nn.relu, name="dense")
        ...
        if self.use_dueling:
            # use RNN output for dueling heads
            value = tf.layers.dense(rnn, units=1, name="dense_value")
            advantage = tf.layers.dense(rnn, units=self.num_actions, use_bias=False, name="dense_advantage")
            qvalues = value + advantage - tf.reduce_sum(advantage, axis=1, keepdims=True)
        else:
            qvalues = tf.layers.dense(rnn, units=self.num_actions, name="dense_q")
    ...
    def _add_to_replay_buffer(self, sample_buffer):
        ...
        for episode in sample_buffer.episodes(): # Each user has its own episode.
            ...
            for step in range(len(episode.states)):  # Step represent the sequence number of the transmitted packet
                ...
                if (episode.states[step] != -1) and (episode.actions[step] != -1) and (episode.rewards[step] != -1):
    ...
    def train(self, sample_buffer, print_every=1000, **kwargs):
        ...
        # compute integer number of batches
        if batch_size * (unroll_step - self.skip_error) == 0:
            return 0, 0
        ...
        batch_terminal = np.zeros((max_,), dtype=np.bool_)
        batch_mask = np.zeros((max_,), dtype=np.float32)

        # calc batch number
        # n_batches already computed above
