       ...
       # env.network.reset_ia()
        ...
        for time_step in range(time_slots):
            ...
            if time_step%(episode_interval) == episode_interval-1:
                ...
                episode_reward_log.append(cum_r[episode_interval])
                episode_collision_log.append(cum_collision[episode_interval])
                runtime_log_rows.append([time_step, float(mainDRQN.get_eps()), float(cum_collision[episode_interval]), float(cum_r[episode_interval]), float(time.time()-start_time)])
                try:
                    save_dir = "save_model/" + "test/"
            ...
            if time_step%save_freq == save_freq-1:
                ...
                if save_results:
                  ...
                  #  np.save(filename, np.asarray(str(time.time()-start_time)))
                    ...
                    filename = save_dir + "/episode_rewards" + "_sim"+str(simulation)
                    np.save(filename, np.asarray(episode_reward_log))
                    filename = save_dir + "/episode_collisions" + "_sim"+str(simulation)
                    np.save(filename, np.asarray(episode_collision_log))
        ...
        if save_results:
            save_dir = "save_results/" + "test/" + experiment_name
            optimal = env.get_num_ch() * episode_interval
            try:
                plot_episode_rewards(save_dir, episode_reward_log, optimal=optimal, label="MARL", filename="training_curve.png")
            except Exception:
                pass
            try:
                csv_path = os.path.join(save_dir, "runtime_log.csv")
                with open(csv_path, "w", newline="") as f:
                    w = csv.writer(f)
                    w.writerow(["time_step","epsilon","cum_collision","sum_reward","total_time"])
                    w.writerows(runtime_log_rows)
            except Exception:
                pass
        # Always save a final snapshot at the end of simulation
        try:
            save_dir = "save_model/" + "test/"
            mainDRQN.save_model(save_dir, time_slots-1, simulation)
        except Exception:
            pass

        # ================= GRAPH GENERATION SECTION =================
        print("\nGenerating Training Graphs for Simulation " + str(simulation + 1) + "...")
        ...
        plt.figure()
        plt.plot(episode_reward_log)
        plt.title("Episode Reward vs Episodes")
        plt.xlabel("Episode")
        plt.ylabel("Total Reward")
        plt.grid(True)
        plt.savefig(save_dir + "/episode_reward_curve_sim" + str(simulation) + ".png")
        plt.close()

        # 2 Episode Collision Curve
        plt.figure()
        plt.plot(episode_collision_log)
        plt.title("Collisions vs Episodes")
        plt.xlabel("Episode")
        plt.ylabel("Collisions")
        plt.grid(True)
        plt.savefig(save_dir + "/collision_curve_sim" + str(simulation) + ".png")
        plt.close()

        # 3 Reward per Time Slot
        plt.figure()
        plt.plot(log_reward_slot)
        plt.title("Reward per Time Slot")
        plt.xlabel("Time Slot")
        plt.ylabel("Reward")
        plt.grid(True)
        plt.savefig(save_dir + "/reward_timeslot_curve_sim" + str(simulation) + ".png")
        plt.close()

        # 4 Epsilon Decay Curve (Exploration -> Exploitation)
        if len(runtime_log_rows) > 0:
            epsilons = [row[1] for row in runtime_log_rows]
            plt.figure()
            plt.plot(epsilons)
            ...
            plt.ylabel("Epsilon")
            plt.grid(True)
            plt.savefig(save_dir + "/epsilon_decay_curve_sim" + str(simulation) + ".png")
            plt.close()

        # 5 Reward vs Collision Comparison
        if len(runtime_log_rows) > 0:
            rewards = [row[3] for row in runtime_log_rows]
            collisions = [row[2] for row in runtime_log_rows]

            plt.figure()
            plt.plot(rewards, label="Reward")
            plt.plot(collisions, label="Collisions")
            plt.title("Reward vs Collisions")
            plt.xlabel("Episode")
            plt.legend()
            plt.grid(True)
            plt.savefig(save_dir + "/reward_vs_collision_sim" + str(simulation) + ".png")
            plt.close()
    ...
    for i in range(len(experiments)):
        ...
        if os.path.exists(cfg_path):
            ...
            config = {
                ...
                "global_reward_avg": False,
                "save_positions": False,
                "enable_channel": False,
                "ia_penalty_enable": False,
                "ia_averaging": False,
                "EnvironmentTest": {
                    "num_users": 4,
                    "num_channels": 3,
                    "mobility": True,
                    "mobility_vary": False,
                    "enable_design_topology": False,
                    "highway_length": 200,
                    "enable_fingerprint": False,
                    "reward_design": 1,
                    "communication_range": 200,
                    "proportional_fair": False,
                    ...
                    "State": {
                        ...
                        "piggybacking": False,
                        "add_position": False,
                        "add_positional_dist": False,
                        "add_positional_dist_piggy": False,
                        "add_positional_dist_type": 1,
                        "num_bins": 10,
                        "add_channel_obs": True
                    }
                },
                "RLAgent": {
                    "batch_size": 32,
        ...
        experiment_name = config.setdefault("experiment_name", "")
        # Ensure we store weights even if the external config disables it
